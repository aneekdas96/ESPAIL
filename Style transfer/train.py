...
def display_img(i,x,style,is_val=False):
    ...
    if is_val:
        ...
        fname = 'image/output/%s_%d_val.png' % (style,i)
    else:
        fname = 'image/output/%s_%d.png' % (style,i)
    imsave(fname, img)
    print('Image saved as', fname)

def get_style_img_path(style):
    return "image/style/"+style+".jpg"
...
def main(args):
    ...
    #checkpoint = keras.callbacks.Callback.ModelCheckpoint('Model.{epoch}.hdf5', monitor='loss', verbose=1, save_best_only=True, mode='min')
    ...
    i=0
    t1 = time.time()
    print ("Debug Mark",img_width," ",img_height)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--content_weight', default = 2, type=float)
    parser.add_argument('--style_weight', default = 2, type=float)
